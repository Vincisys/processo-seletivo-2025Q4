...
class TestOwnerRoutes:
    ...
    def test_create_owner_duplicate_email(self, client, created_owner, sample_owner_data):
    ...
    def test_create_owner_invalid_email(self, client):
    ...
    def test_create_owner_missing_field(self, client):
    ...
    def test_get_owner_success(self, client, created_owner):
        """Testa busca bem-sucedida de owner por ID"""
        response = client.get(f"/integrations/owner/{created_owner['id']}")
    ...
    def test_get_owner_not_found(self, client):
        """Testa busca de owner inexistente"""
        fake_id = "00000000-0000-0000-0000-000000000000"
        response = client.get(f"/integrations/owner/{fake_id}")
    ...
    def test_list_owners_with_data(self, client, created_owner):
    ...
    def test_list_owners_pagination(self, client, sample_owner_data):
        """Testa paginação da listagem"""
        # Criar 5 owners
        for i in range(5):
            data = {**sample_owner_data, "email": f"owner{i}@empresa.com"}
            client.post("/integrations/owner", json=data)
        
        # Primeira página
        response = client.get("/integrations/owners?skip=0&limit=2")
        assert response.status_code == 200
        assert len(response.json()) == 2
        
        # Segunda página
        response = client.get("/integrations/owners?skip=2&limit=2")
        assert response.status_code == 200
        assert len(response.json()) == 2
    
    def test_update_owner_success(self, client, created_owner):
    ...
    def test_update_owner_not_found(self, client):
        ...
        fake_id = "00000000-0000-0000-0000-000000000000"
        update_data = {"phone": "+55 11 99999-9999"}
        response = client.put(f"/integrations/owner/{fake_id}", json=update_data)
        
        assert response.status_code == 404
    
    def test_update_owner_invalid_email(self, client, created_owner):
    ...
    def test_delete_owner_success(self, client, created_owner):
        """Testa deleção bem-sucedida de owner"""
        response = client.delete(f"/integrations/owner/{created_owner['id']}")
    ...
    def test_delete_owner_not_found(self, client):
        ...
        fake_id = "00000000-0000-0000-0000-000000000000"
        response = client.delete(f"/integrations/owner/{fake_id}")
        
        assert response.status_code == 404
    
    def test_delete_owner_cascades_to_assets(self, client, created_owner, created_asset):
        """Testa que deletar owner deleta seus assets (CASCADE)"""
        owner_id = created_owner["id"]
        asset_id = created_asset["id"]
        
        # Deletar owner
        response = client.delete(f"/integrations/owner/{owner_id}")
        assert response.status_code == 204
